...
public object Keccak {
    ...
    private fun roundB(state: Array<Array<BigInteger>>) {
        ...
        for (round in 0..23) {
            ...
            for (i in 0..4) {
                c[i] =
                    state[i][0].xor(state[i][1]).xor(state[i][2]).xor(state[i][3]).xor(state[i][4])
            }
    ...
    private fun BigInteger.leftRotate64(rotate: Int) =
        (this shr (64 - rotate)).add(this shl rotate).mod(BIT_65)
}
 ...
 */
private fun IntArray.fillWith(value: Int, fromIndex: Int = 0, toIndex: Int = this.size) {
 ...
 */
private class ArrayIndexOutOfBoundsException(index: Int) :
    Throwable("Array index out of range: $index")
 ...
 */
public enum class KeccakParameter constructor(
    public val rateInBytes: Int,
    public val outputLengthInBytes: Int,
    public val d: Int
) {
